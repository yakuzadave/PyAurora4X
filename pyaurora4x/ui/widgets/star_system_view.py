...
from pyaurora4x.core.models import StarSystem, Fleet, Planet, AsteroidBelt
...
class StarSystemView(Static):
    ...
    def update_system(
        self, star_system: StarSystem, fleets: Optional[List[Fleet]] = None
    ) -> None:
        """
    ...
    def _calculate_scale(self) -> None:
        ...
        distances = [p.orbital_distance for p in self.system.planets]
        distances += [b.distance for b in self.system.asteroid_belts]
        if not distances:
            self.scale_factor = 1.0
            return
        ...
        if max_distance > 0:
            self.scale_factor = self.max_radius / (
                max_distance * 149597870.7
            )  # Convert AU to km
        else:
            self.scale_factor = 1.0
    ...
    def _render_system(self) -> None:
        ...
        # Draw asteroid belts
        for belt in self.system.asteroid_belts:
            self._draw_asteroid_belt(display, belt)
    ...
    def _get_star_character(self, star_type) -> str:
        ...
        return star_chars.get(
            star_type.value if hasattr(star_type, "value") else str(star_type), "☉"
        )

    def _draw_planet_orbit(
        self, display: List[List[str]], planet: Planet, index: int
    ) -> None:
        ...
        for i in range(points):
            ...
            y = int(
                self.center_y + orbit_radius_display * math.sin(angle) * 0.5
            )  # Flatten for ASCII
    ...
    def _draw_asteroid_belt(self, display: List[List[str]], belt: AsteroidBelt) -> None:
        """Draw an asteroid belt as a dashed circle."""
        radius_km = belt.distance * 149597870.7
        radius_display = radius_km * self.scale_factor
        if radius_display < 1 or radius_display > self.max_radius:
            return
        ...
        for i in range(points):
            if i % 2 == 0:
                continue
            angle = (2 * math.pi * i) / points
            x = int(self.center_x + radius_display * math.cos(angle))
            y = int(self.center_y + radius_display * math.sin(angle) * 0.5)
            if 0 <= y < len(display) and 0 <= x < len(display[0]):
                if display[y][x] == " ":
                    display[y][x] = ":"
    ...
    def _draw_planet(
        ...
        y_display = int(
            self.center_y + y_km * self.scale_factor * 0.5
        )  # Flatten for ASCII
    ...
    def _get_planet_character(self, planet: Planet, index: int) -> str:
        ...
        planet_type = (
            ...
            if hasattr(planet.planet_type, "value")
            else str(planet.planet_type)
        )
    ...
    def _generate_system_info(self) -> str:
        ...
        info_lines.append(
            f"Star: {self.system.star_type.value if hasattr(self.system.star_type, 'value') else self.system.star_type}"
        )
        ...
        for i, planet in enumerate(self.system.planets):
            ...
            info_lines.append(
                f"  {i+1}. {planet.name} ({planet_type}) - {distance_str}"
            )

        if self.system.asteroid_belts:
            info_lines.append("")
            info_lines.append(f"Asteroid Belts: {len(self.system.asteroid_belts)}")
            for i, belt in enumerate(self.system.asteroid_belts):
                info_lines.append(
                    f"  Belt {i+1}: {belt.distance:.2f} AU width {belt.width:.2f} AU"
                )
        ...
        if self.system_fleets:
            ...
            for fleet in self.system_fleets:
                status = (
                    fleet.status.value
                    if hasattr(fleet.status, "value")
                    else str(fleet.status)
                )
